...
locals {
    ...
    run_devops_command = <<EOT
        ssh -l ${module.blueprint_devops_self_hosted_agent.object.admin_username} ${module.blueprint_devops_self_hosted_agent.object.public_ip_address} << EOF
            docker run -d -e AZP_URL=${var.azure_devops_url_organization} -e AZP_TOKEN=${var.azure_devops_pat_token} -e AZP_POOL="${var.azure_devops_agent_pool}" ${module.blueprint_container_registry.object.login_server}/devops
